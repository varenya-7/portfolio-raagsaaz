  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        transition={{ duration: 0.3 }}
        className="fixed inset-0 bg-zinc-800 bg-opacity-80 flex items-center justify-center z-50 p-4 overflow-auto"
        onClick={onClose}
      >
        <motion.div
          onClick={(e) => e.stopPropagation()}
          className="bg-black rounded-2xl max-w-lg w-full relative p-4 md:p-6"
          initial={{ opacity: 0, y: 100 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ opacity: 0, y: 100 }}
          transition={{ type: "spring", stiffness: 80, damping: 18 }}
        ...
        >
          ...
          <p className="text-gray-400 mb-2">{member.instrument}</p>
